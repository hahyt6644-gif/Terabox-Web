...
async function getFileInfo(link, event, cookie) {
  try {
    ...
    return {
      ...
      proxy_url: `https://dark-bonus-5f8f.amitkr545545.workers.dev/proxy?url=${encodeURIComponent(fileInfo.dlink)}&file_name=${encodeURIComponent(fileInfo.server_filename || 'download')}&cookie=${encodeURIComponent(cookie)}`,
    };
  } catch (error) {
